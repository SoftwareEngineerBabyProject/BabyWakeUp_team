...
import com.example.wenyu.baby.preferences.AlarmPreference.Type;
...
public class AlarmPreferenceListAdapter extends BaseAdapter implements Serializable {
	...
	private final String[] alarmDifficulties = {"Cockroach","Type","Shake","Guess","Math"};
...
/////////////
	...
	public void setMathAlarm(Alarm alarm) {
		this.alarm = alarm;
		preferences.clear();
		preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_ACTIVE, "鬧鐘設定", null, null, alarm.getAlarmActive(), Type.BOOLEAN));
		preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_VIBRATE, "震動", null, null, alarm.getVibrate(), Type.BOOLEAN));
		preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_TIME, "時間",alarm.getAlarmTimeString(), null, alarm.getAlarmTime(), Type.TIME));
		preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_REPEAT, "重覆星期",alarm.getRepeatDaysString(), repeatDays, alarm.getDays(),Type.MULTIPLE_LIST));
		preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_DIFFICULTY,"響起遊戲", alarm.getDifficulty().toString(), alarmDifficulties, alarm.getDifficulty(), Type.LIST));
...
//////鈴聲/////
		...
		if(alarmTone instanceof Ringtone && !alarm.getAlarmTonePath().equalsIgnoreCase("")){
			preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_TONE, "鈴聲", alarmTone.getTitle(getContext()),alarmTones, alarm.getAlarmTonePath(), Type.LIST));
		}else{
			preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_TONE, "鈴聲", getAlarmTones()[0],alarmTones, null, Type.LIST));
		}*/
//////鈴聲/////
